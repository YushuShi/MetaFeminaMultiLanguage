...
class SummaryPageTests(unittest.TestCase):
    ...
    def test_requested_subcategory_plot_rules_are_encoded(self):
        ...
        self.assertIn('entry["headline"].get("n_studies", 0) >= FORMAL_EGGER_MIN_STUDIES', analysis_source)
        self.assertIn('entry["headline"]["i2"] > 0', analysis_source)
        self.assertIn('min_studies = 3', paper_plot_source)
        self.assertIn('EGGER_MIN_STUDIES <- 10', paper_plot_source)
        self.assertIn('min_studies  = EGGER_MIN_STUDIES', paper_plot_source)
